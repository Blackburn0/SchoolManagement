...
from django.contrib.auth.models import Group, auth, User
...
def student_signup_view(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            messages.info(request, 'Huff ,Username was already taken')
            return redirect("studentsignup")

        elif User.objects.filter(email=email).exists():
            messages.info(request, 'Come on, Email already exist')
            return redirect("studentsignup")
            ...
            user.save()
            html_template = 'school/studentup_email.html'
            ...
            message.send()
            return redirect("school/studentsignupconfirmation.html")

    else:
        return render(request, 'school/studentsignup.html')

    #     return HttpResponseRedirect('studentsignupconfirmation')
